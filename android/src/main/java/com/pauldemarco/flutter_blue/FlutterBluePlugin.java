...
public class FlutterBluePlugin implements FlutterPlugin, ActivityAware, MethodCallHandler, RequestPermissionsResultListener  {
    ...
    public void onMethodCall(MethodCall call, Result result) {
        ...
        switch (call.method) {
            ...
            {
                byte[] data = call.arguments();
                //log(LogLevel.DEBUG,"writeCharacteristic: data length = "+data.length);
                ...
                //log(LogLevel.DEBUG,"writeCharacteristic: data length2 = "+request.getValue().toByteArray().length);
                //log(LogLevel.DEBUG,"writeCharacteristic: data length3 = "+request.getValue().toByteArray());
                ...
                waitIdle(1000);
                //log(LogLevel.DEBUG, "[writeCharacteristic] ok rwBusy: " + rwBusy );
                result.success(null);
                break;
            }
...
//            }
    ...
    public boolean onRequestPermissionsResult(
        ...
        try {
            if (requestCode == REQUEST_FINE_LOCATION_PERMISSIONS && grantResults.length>0) {
                if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    if(pendingResult!=null && pendingCall!=null)startScan(pendingCall, pendingResult);
                } else {
                    if(pendingResult!=null) pendingResult.error("no_permissions", "flutter_blue plugin requires location permissions for scanning", null);
                    pendingResult = null;
                    pendingCall = null;
                }
                return true;
...
//                    }
                ...
                if (newState == BluetoothProfile.STATE_CONNECTED && Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
